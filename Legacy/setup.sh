...
# Profile photos are stored outside the public document root and served by WebApp/functions/photo.php.
PHOTO_STORAGE_DIR="${NIGHTWATCH_PHOTO_STORAGE_DIR:-${PROJECT_DIR}/storage/photos}"
...
[[ "${SERVER_NAME}" =~ ^[A-Za-z0-9.-]+$ ]] || { echo "SERVER_NAME may contain only letters, numbers, dots, and hyphens." >&2; exit 1; }
[[ "${SITE_NAME}" =~ ^[A-Za-z0-9._-]+\.conf$ ]] || { echo "SITE_NAME must be a simple .conf filename." >&2; exit 1; }
...
install -d -o "${APACHE_USER}" -g "${APACHE_GROUP}" -m 0750 "${PHOTO_STORAGE_DIR}"
printf 'Require all denied\n' > "${PHOTO_STORAGE_DIR}/.htaccess"
chown root:"${APACHE_GROUP}" "${PHOTO_STORAGE_DIR}/.htaccess"
chmod 0644 "${PHOTO_STORAGE_DIR}/.htaccess"

# Create legacy upload directories for backward-compatible photo serving; new uploads go to private storage.
...
# Lock application source files as read-only for Apache; only private photos, legacy uploads, and logs remain writable.
...
chown -R "${APACHE_USER}":"${APACHE_GROUP}" "${WEB_ROOT}/uploads" "${LOG_DIR}" "${PHOTO_STORAGE_DIR}"
find "${WEB_ROOT}/uploads" -type d -exec chmod 0755 {} \;
find "${WEB_ROOT}/uploads" -type f -exec chmod 0644 {} \;
chmod 0700 "${LOG_DIR}"
chmod 0750 "${PHOTO_STORAGE_DIR}"
chmod 0644 "${PHOTO_STORAGE_DIR}/.htaccess"
...

{
  ...
  [[ -n "${SMTP_VERIFY_CERT:-}" ]] && apache_setenv SMTP_VERIFY_CERT "${SMTP_VERIFY_CERT}"
  [[ -n "${SMTP_REQUIRE_AUTH:-}" ]] && apache_setenv SMTP_REQUIRE_AUTH "${SMTP_REQUIRE_AUTH}"
  [[ -n "${SMTP_ALLOW_INSECURE_AUTH:-}" ]] && apache_setenv SMTP_ALLOW_INSECURE_AUTH "${SMTP_ALLOW_INSECURE_AUTH}"
  ...
  apache_setenv NIGHTWATCH_LOG_DIR "${LOG_DIR}"
  apache_setenv NIGHTWATCH_PHOTO_STORAGE_DIR "${PHOTO_STORAGE_DIR}"
  [[ -n "${NIGHTWATCH_PHOTO_PUBLIC_BASE_URL:-}" ]] && apache_setenv NIGHTWATCH_PHOTO_PUBLIC_BASE_URL "${NIGHTWATCH_PHOTO_PUBLIC_BASE_URL}"
  [[ -n "${NIGHTWATCH_REQUIRE_MODERATION:-}" ]] && apache_setenv NIGHTWATCH_REQUIRE_MODERATION "${NIGHTWATCH_REQUIRE_MODERATION}"
  [[ -n "${NIGHTWATCH_ALLOW_INSECURE_DEFAULTS:-}" ]] && apache_setenv NIGHTWATCH_ALLOW_INSECURE_DEFAULTS "${NIGHTWATCH_ALLOW_INSECURE_DEFAULTS}"
} > "${ENV_CONF}"
...
echo "Legacy uploads: ${WEB_ROOT}/uploads/photos"
echo "Private profile photos: ${PHOTO_STORAGE_DIR}"
echo "Logs: ${LOG_DIR}"
