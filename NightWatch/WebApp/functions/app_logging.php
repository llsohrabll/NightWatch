...
{
    $candidates = [
        dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'logs',
        sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'nightwatch_logs',
    ];
 ...
 */
function log_security_event(string $event, string $level = 'INFO', ?string $userId = null): void
{
    ...
    $safeLevel = log_safe_value($level);
    $safeUserId = $userId !== null ? log_safe_value($userId) : 'anonymous';
    $safeEvent = log_safe_value($event);
