...
// ============================================================================
// Attempts to use PHPMailer if available, otherwise uses native SMTP via mail()
function send_mail(string $to, string $subject, string $textMessage, string $htmlMessage = ''): bool
{
    ...
    if ((string) SMTP_USERNAME === '' || (string) SMTP_PASSWORD === '') {
        log_mail_event($recipient, $safeSubject, false, 'SMTP_USERNAME or SMTP_PASSWORD is not configured');
        return false;
    }
...
function send_mail_via_mail(string $recipient, string $fromAddress, string $fromName, string $subject, string $textMessage, string $htmlMessage = ''): bool
{
    try {
        ...
        // Read greeting
        $response = stream_get_line($smtp, 1024, "\r\n");
        ...
        while (true) {
            $line = stream_get_line($smtp, 1024, "\r\n");
