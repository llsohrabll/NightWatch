...
foreach ($passwordErrors as $passwordError) {
    $errors[] = str_replace('Password ', 'New password ', $passwordError);
}
if (strpos($confirmPassword, "\0") !== false) {
    $errors[] = 'Password confirmation contains invalid characters.';
}
...
if ($expiresAtTimestamp === false || $now > $expiresAtTimestamp) {
    $stmt->close();
    $conn->close();
...
$updateStmt = $conn->prepare("
    UPDATE users 
    SET password = ?, reset_code = NULL, reset_code_expires_at = NULL, token = NULL, token_expires_at = NULL, login_failures = 0, locked_until = NULL
    WHERE id = ?
");
