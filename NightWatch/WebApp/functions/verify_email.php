...
$verificationCode = trim(strip_null_bytes((string) ($_POST['verification_code'] ?? '')));
...
// ============================================================================
if (!nightwatch_verify_one_time_code((string) $user['email_verification_code'], $verificationCode)) {
    $stmt->close();
    $conn->close();
    
    log_suspicious_activity("INVALID_EMAIL_VERIFICATION_CODE | User: {$user['id']} | Email: {$user['email']}", (string) $user['id']);
...
// ============================================================================
log_security_event(
    "EMAIL_VERIFIED | User: {$user['id']} | Email: {$user['email']}",
    'INFO',
    (string) $user['id']
);

// ============================================================================
// STEP 6: Now that the account is verified, issue the browser a real auth token.
// ============================================================================
$sessionExpiresAt = issue_user_token($conn, $user, NIGHTWATCH_SESSION_TTL);
