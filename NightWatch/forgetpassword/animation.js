(function() {
    ...
    // کلاس ستاره‌های ثابت و صورت فلکی
    class Particle {
    ...
    // کلاس جدید و طبیعی‌تر ستاره دنباله‌دار
    class ShootingStar {
        constructor() {
            this.reset();
        }
        ...
        reset() {
            // شروع از بیرون صفحه (بالا یا راست)
            this.x = Math.random() * width + width / 2; 
            this.y = -Math.random() * height;
            
            this.length = Math.random() * 150 + 100; // دم بسیار بلندتر
            this.speed = Math.random() * 20 + 25;    // سرعت بسیار زیاد (طبیعی‌تر)
            this.size = Math.random() * 1.5 + 0.5;   // ضخامت
            this.waitTime = Math.random() * 300 + 100; // زمان انتظار بین هر شهاب
            this.active = false;
            this.angle = Math.PI / 4; // زاویه ۴۵ درجه به سمت پایین-چپ
        }
        
        update() {
            if (this.active) {
                // حرکت سریع در امتداد زاویه
                this.x -= Math.cos(this.angle) * this.speed;
                this.y += Math.sin(this.angle) * this.speed;
                
                // اگر از کادر خارج شد، ریست شود تا دوباره بعداً ظاهر شود
                if (this.x < -this.length || this.y > height + this.length) {
                    this.reset();
                }
            } else {
                // شمارش معکوس برای ظاهر شدن
                this.waitTime--;
                if (this.waitTime <= 0) {
                    this.active = true;
                }
        ...
        draw() {
            ...
            // محاسبه انتهای دم شهاب‌سنگ
            const tailX = this.x + Math.cos(this.angle) * this.length;
            const tailY = this.y - Math.sin(this.angle) * this.length;
            
            // ساخت گرادینت برای دم (از سفید به نامرئی)
            const gradient = ctx.createLinearGradient(this.x, this.y, tailX, tailY);
            gradient.addColorStop(0, "rgba(255, 255, 255, 1)");      // سر شهاب کاملا سفید
            gradient.addColorStop(0.1, "rgba(176, 199, 233, 0.4)");  // بدنه کمرنگ
            gradient.addColorStop(1, "rgba(176, 199, 233, 0)");      // انتهای دم محو
            ...
            // ریست کردن سایه برای اینکه روی بقیه ستاره‌ها تاثیر نذاره
            ctx.shadowBlur = 0; 
        }
    }
        ...
        for (let i = 0; i < particles.length; i++) {
            for (let j = i + 1; j < particles.length; j++) {
                ...
                if (distance < connectionDistance) {
                    ...
                    ctx.strokeStyle = `rgba(176, 199, 233, ${opacity * 0.15})`;
                    ctx.beginPath();
    ...
    function initParticles() {
        ...
        // ساخت ستاره‌های صورت فلکی
        for (let i = 0; i < particleCount; i++) {
            particles.push(new Particle());
        }
        
        // ساخت ۲ شهاب‌سنگ که به نوبت در صفحه ظاهر میشن
        for (let i = 0; i < 2; i++) {
    ...
    function drawLines() {
        ...
        for (let i = 0; i < particles.length; i++) {
            for (let j = i + 1; j < particles.length; j++) {
                ...
                if (dist < 150) {
                    ctx.strokeStyle = `rgba(176, 199, 233, ${0.15 * (1 - dist/150)})`;
                    ctx.beginPath();
                    ctx.moveTo(particles[i].x, particles[i].y);
                    ctx.lineTo(particles[j].x, particles[j].y);
                    ctx.stroke();
