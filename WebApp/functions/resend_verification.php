...
$userId = (int) $user['id'];
$updateStmt->bind_param('sii', $verificationCode, $expiresAt, $userId);
...
@file_put_contents(
    $logFile,
    date('Y-m-d H:i:s') . " | Verification code resend requested for user: {$userId} ({$user['email']}) | Email sent: " . ($emailSent ? 'YES' : 'NO') . " | IP: " . request_ip_address() . "\n",
    FILE_APPEND
);
