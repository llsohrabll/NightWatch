(function () {
  ...
  class Star {
    ...
    update() {
      ...
      if (pointer.active && dist < 135 && dist > 0.01) {
        const pull = (135 - dist) / 135;
        this.x -= (dx / dist) * pull * 0.38;
        this.y -= (dy / dist) * pull * 0.38;
      }
  ...
  function drawPointerHalo() {
    ...
    const gradient = ctx.createRadialGradient(pointer.x, pointer.y, 0, pointer.x, pointer.y, 145);
    gradient.addColorStop(0, 'rgba(125, 231, 255, 0.13)');
    gradient.addColorStop(0.55, 'rgba(196, 155, 255, 0.045)');
    gradient.addColorStop(1, 'rgba(125, 231, 255, 0)');
