...
(function () {
    ...
    update() {
      ...
      if (pointer.active) {
        ...
        if (distance < 150 && distance > 0.01) {
          ...
          this.x -= (dx / distance) * force * 0.34;
          this.y -= (dy / distance) * force * 0.34;
        }
      }
  ...
  function drawPointerGlow() {
    ...
    glow.addColorStop(0, 'rgba(121, 232, 255, 0.11)');
    glow.addColorStop(0.45, 'rgba(168, 139, 255, 0.04)');
    glow.addColorStop(1, 'rgba(121, 232, 255, 0)');
    ctx.fillStyle = glow;
    ctx.beginPath();
  ...
  function initTilt() {
    const cards = document.querySelectorAll('.stat-card, .mission-card, .session-card, .activity-log, .top-bar');
    cards.forEach((card) => {
